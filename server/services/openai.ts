...
import { AIInsights, NewsItem, RankedNewsItem, NewsRankingResponse } from '@shared/schema';
...
export class OpenAIService {
  ...
  async rankNewsByImpact(newsItems: NewsItem[], instrumentContext?: string): Promise<NewsRankingResponse> {
    try {
      ...
      const openaiPromise = this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "You are a financial risk analyst specializing in commodity and currency markets. Your job is to quickly assess news articles for their potential market impact, particularly focusing on Southeast Asian markets. Be concise and actionable in your analysis."
          },
          {
            role: "user", 
            content: prompt
        ...
        for (const ranking of result.rankings) {
          ...
          if (originalItem && ranking.riskScore >= 4) {
            rankedItems.push({
              ...
              impactReason: ranking.impactReason || 'Impact assessment unavailable'
            });
          }
        }
      }
