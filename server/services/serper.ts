...
export type RegionCode = typeof NEWS_REGIONS[keyof typeof NEWS_REGIONS]['code'];
...
export class SerperService {
  ...
  async searchNewsByRegion(query: string, region: RegionCode, timeFilter?: string): Promise<NewsResponse> {
    try {
      ...
      console.error(`Error fetching news from Serper for region ${region}:`, error);
      throw new Error(`Failed to fetch news data for region ${region}`);
    }
  }

  async collectGlobalNews(query: string, timeFilter: string = NEWS_TIME_FILTER): Promise<NewsResponse> {
    ...
    try {
      ...
      const regionalNewsPromises = regionCodes.map(region => 
        this.searchNewsByRegion(query, region, timeFilter)
          .catch(error => {
  ...
  async triageAndRankNews(query: string, instrumentContext?: string): Promise<NewsRankingResponse> {
    try {
  ...
  async getInstrumentNewsWithRanking(instrumentName: string): Promise<NewsRankingResponse> {
    const searchQuery = `${instrumentName} commodity currency market news Southeast Asia Thailand`;
